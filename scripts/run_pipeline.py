...
K = np.array(
    [[1013.535131, 0.0, 638.295572], [0.0, 1060.724658, 399.313666], [0.0, 0.0, 1.0]]
)
...
def parse_args():
    ...
    parser.add_argument("--clouds-dir", type=Path, default=Path("clouds"))
...
def main():
    args = parse_args()
    images = [
        cv.imread(str(p)) for p in sorted(args.image_dir.glob("*.png"))[:: args.step]
    ]
    ...
    config = SLAMConfig(
        ...
        bundle_adjustment=LocalBundleAdjustment(
            LocalBundleAdjustmentConfig(
                window_size=10, max_iterations=20, huber_radius=1.0
            )
        ),
