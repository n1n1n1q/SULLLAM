...
class HomographyPoseEstimator(BaseEstimator):
    ...
    def _estimate(self, keypoints_query, keypoints_train, matches):
        ...
        src_pts = np.float32([keypoints_query[m.queryIdx].pt for m in matches]).reshape(
            -1, 1, 2
        )
        dst_pts = np.float32([keypoints_train[m.trainIdx].pt for m in matches]).reshape(
            -1, 1, 2
        )
        ...
        inlier_matches = [
            m for m, is_inlier in zip(matches, inlier_mask_flat) if is_inlier
        ]
