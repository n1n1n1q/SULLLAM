...
def estimate_homography_fallback(
    kp_query, kp_train, matches, ransac_reproj_threshold: float = 3.0
):
    ...
    homography, inlier_mask = cv.findHomography(
        src_pts, dst_pts, cv.RANSAC, ransac_reproj_threshold
    )
...
def main() -> None:
    ...
    parser.add_argument("train_image", type=Path)
    ...
    print(
        f"Success: {result.get('success', False)} ({result.get('reason', 'unknown')})"
    )
