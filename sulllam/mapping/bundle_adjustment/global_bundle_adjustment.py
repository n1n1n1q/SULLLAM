...
class GlobalBundleAdjustment(BaseBundleAdjustment):
    ...
    def run(self, mapper, K: np.ndarray) -> None:
        ...
        if len(local_point_ids) == 0:
            ...
            return
        print(
            f"[GBA] {len(mapper.keyframes)} keyframes, {len(local_point_ids)} points, {len(ba_obs)} observations"
        )
        ...
        if mean_err_per_obs > cfg.max_mean_error_per_obs:
            print(
                f"[GBA] Mean error per obs {mean_err_per_obs:.1f} exceeds threshold {cfg.max_mean_error_per_obs:.1f}. Skipping to avoid divergence."
            )
        ...
        print(
            f"[GBA] Final error : {final_error:.4f}  (Δ {initial_error - final_error:.4f})"
        )
