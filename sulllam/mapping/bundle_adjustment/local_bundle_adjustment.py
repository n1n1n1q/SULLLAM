...
import cv2
from sulllam.mapping.bundle_adjustment.base_bundle_adjustment import (
    BaseBundleAdjustment,
)
...
class LocalBundleAdjustmentConfig:
    ...
    abs_err_tolerance: float = 0.0001
    rel_err_tolerance: float = 0.0001
...
class LocalBundleAdjustment(BaseBundleAdjustment):
    ...
    def run(self, mapper, K: np.ndarray) -> None:
        ...
        if len(local_point_ids) == 0:
            ...
            return
        external_obs_mask = np.isin(all_obs[:, 0], local_point_ids) & ~np.isin(
            all_obs[:, 1], list(local_kf_ids)
        )
        external_obs = all_obs[external_obs_mask]
        external_kf_ids = set(np.unique(external_obs[:, 1]).astype(int).tolist())
        print(
            f"[BA] {len(local_point_ids)} points, {len(ba_obs)} window obs, {len(external_obs)} external obs from {len(external_kf_ids)} fixed external KFs"
        )
        local_kfs_sorted = sorted(local_kfs, key=lambda k: k.idx)
        n_fixed = max(2, cfg.num_fixed_keyframes)
        n_fixed = min(n_fixed, len(local_kfs_sorted))
        fixed_kf_ids_in_window = {kf.idx for kf in local_kfs_sorted[:n_fixed]}
        print(
            f"[BA] Anchored (fixed) window keyframes: {sorted(fixed_kf_ids_in_window)}"
        )
        kf_scores = {}
        if cfg.use_match_confidence:
            ...
            if kf_scores:
                print(
                    f"[BA] Using match confidence weights from {len(kf_scores)} keyframes"
                )
        kf_entropies = {}
        alpha_global = 0.0
        if cfg.use_adaptive_barron:
            for kf in local_kfs:
                if hasattr(kf, "image") and kf.image is not None:
                    gray = (
                        cv2.cvtColor(kf.image, cv2.COLOR_BGR2GRAY)
                        if len(kf.image.shape) == 3
                        else kf.image
                    )
            ...
            if kf_entropies:
                ...
                alpha_global = map_entropy_to_alpha(
                    mean_entropy, cfg.barron_alpha_min, cfg.barron_alpha_max
                )
                print(
                    f"[BA] Adaptive Barron: entropy {mean_entropy:.3f} → alpha {alpha_global:.3f}"
                )
        K_tensor = torch.tensor(
            [[K[0, 0], K[1, 1], K[0, 2], K[1, 2]]], dtype=torch.float64
        )
        ...
        for ext_id in external_kf_ids:
            ...
            if kf is None:
                continue
            pose_tensor = torch.from_numpy(kf.pose[:3, :]).unsqueeze(0).double()
            external_se3_vars[ext_id] = th.SE3(
                tensor=pose_tensor, name=f"cam_ext_{ext_id}"
            )
        pt_vars: dict[int, th.Point3] = {}
        for pt_id in local_point_ids:
            pt_tensor = (
                torch.from_numpy(mapper.pointmap.points_3d[pt_id]).unsqueeze(0).double()
            )
        ...
        for o in ba_obs:
            ...
            edge_weight = th.ScaleCostWeight(
                torch.tensor(weight_scalar, dtype=torch.float64)
            )
        ...
        print(
            f"[BA] Graph: {fixed_edges} fixed-cam edges (window), {opt_edges} opt-cam edges, {ext_edges} external fixed-cam edges"
        )
        objective.update()
        ...
        print(
            f"[BA] Final error : {final_error:.4f}  (Δ {initial_error - final_error:.4f})"
        )
