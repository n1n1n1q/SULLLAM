...
class LoopClosureDetector:
    ...
    def detect(self, current_kf: Keyframe, mapper: Mapper, K: np.ndarray) -> list[dict]:
        cfg = self.config
        candidates = []
        ...
        for c in candidates:
            print(
                f"[LC] Loop closure: kf {c['query_kf'].idx} <-> kf {c['match_kf'].idx}  ({c['num_inliers']} inliers, ratio {c['inlier_ratio']:.2f}, rot {c['rot_angle_deg']:.1f} deg)"
            )
        return candidates
