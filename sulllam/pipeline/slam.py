...
_PER_KEYPOINT_KEYS = frozenset(
    {"keypoints", "keypoint_scores", "descriptors", "scales", "oris"}
)


def _filter_by_mask(
    kps: list, descs: np.ndarray, feats: dict | None, mask: np.ndarray
) -> tuple[list, np.ndarray, dict | None, list[int]]:
    h, w = mask.shape[:2]
    keep = [
        i
        for i, kp in enumerate(kps)
        if not mask[min(int(kp.pt[1]), h - 1), min(int(kp.pt[0]), w - 1)]
    ]
...
class SLAMPipeline:
    ...
    def _process_frame(self, image: np.ndarray) -> dict:
        ...
        if (
            ...
            matches, self._current_match_scores = cfg.matcher.match_tensors(
                self._prev_feats, curr_feats
            )
        else:
            matches, self._current_match_scores = cfg.matcher.match(
        ...
        if not self._is_keyframe(n_inliers):
            print(
                f"[SLAM] Frame {i}: skipped (inliers={n_inliers}, frames_since_kf={i - self._last_kf_frame_idx})"
            )
            self._frame_idx += 1
            return {
                "matches": matches,
        ...
        if cfg.segmentor is not None:
            ...
            curr_descs = (
                curr_descs[keep_for_kf] if curr_descs is not None else curr_descs
            )
            if curr_feats is not None:
                curr_feats = {
                    k: (
                        v[keep_for_kf]
                        if isinstance(v, torch.Tensor) and k in _PER_KEYPOINT_KEYS
                        else v
                    )
            ...
            print(
                f"[SLAM] Frame {i}: keyframe — {n_inliers - len(inlier_matches)} keypoints removed by SAM segmentation"
            )
        ...
        track_list: list[tuple[int, int, int, int]] = []
        ...
        if new_indices:
            ...
            for cand in candidates:
                ...
                pt_id = self.mapper.pointmap.add_point(
                    cand["pt3d"], color=cand["color"]
                )
                self.mapper.pointmap.add_observation(
                    pt_id, prev_kf_map.idx, cand["uv_prev"]
                )
                self.mapper.pointmap.add_observation(
                    pt_id, curr_kf.idx, cand["uv_curr"]
                )
